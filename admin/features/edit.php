            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Edit features</span></h4>
                                                ...
                                                if ($file !== "") {
                                                    if ($file_size <= 2 * 1024 * 1024) {
                                                        ...
                                                        if (in_array($extlink, ['jpg', 'png', 'jpeg'])) {
                                                            $oldfilelink = $data['icon'];
                                                            if ($oldfilelink != '' && file_exists('../uploads/' . $oldfilelink)) {
                                                                unlink('../uploads/' . $oldfilelink);
                                                            }

                                                            if (move_uploaded_file($_FILES['icon']['tmp_name'], $target_file)) {
                                                                $query = "UPDATE features SET title='$title', description='$description', icon='$finalname', status='$status', updated_at=NOW() WHERE id=$id";
                                                    ...
                                                    $result = mysqli_query($con, $query);
                                                    if ($result) {
                                                        echo "<div class='alert alert-success'>features updated successfully.</div>";
                                                        echo "<meta http-equiv=\"refresh\" content=\"2;URL=index.php\">";
                                                    } else {
                                                        echo "<div class='alert alert-danger'>Failed to update features.</div>";
                                                        echo "<meta http-equiv=\"refresh\" content=\"2;URL=index.php\">";
                                                    }
                                                }
                                            } else {
